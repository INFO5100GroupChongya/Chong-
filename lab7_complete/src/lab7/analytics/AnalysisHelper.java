...
// Question 3
    ...
    public void getFiveInactiveUsersPosts() {
        ...
        for(int i=0;i<userPostCountList.size()&&i<5;i++){
            System.out.println("No."+(i+1)+"  UserID:"+userPostCountList.get(i).getKey()+" PostNumber: "+userPostCountList.get(i).getValue()); 
        }
    }
    ...
    //Key:UserId Value:total comments /user;
    
    public void getFiveInactiveUsersComments(){
         ...
         for(User user: users.values()){
             ...
             for(Comment c: comments.values()){
                 if(user.getId() == c.getUserId()){
                     count++;
                     userCommentsCount.put(user.getId(), count);
                 }
              
             }
         }
         
         //sort total comment number.
         List<Map.Entry<Integer,Integer>> userCommnetsCountList = new ArrayList<>(userCommentsCount.entrySet());
         Collections.sort(userCommnetsCountList, new Comparator<Map.Entry<Integer,Integer>>() {
             @Override
             public int compare(Map.Entry<Integer, Integer> o1, Map.Entry<Integer, Integer> o2) {
                 return o1.getValue() - o2.getValue();
             }
        ...
        for(int i=0;i<userCommnetsCountList.size()&&i<5;i++){
            System.out.println("No."+(i+1)+"  UserID:"+userCommnetsCountList.get(i).getKey()+" CommentsNumber: "+userCommnetsCountList.get(i).getValue()); 
        }
    }
